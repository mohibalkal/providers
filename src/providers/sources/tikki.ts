...
// Provider: Tikki ✨  |  id: tikki  |  type: source
// Original location: vendor.js line 20913
...
const lZ = "https://stream.fontaine.lol";
const fZ = "b356d2c1661365ed46513c8adb04b9d85db4857149f936e5e56dcdac3e67a5dd";
function n9(c) {
  const g = new Uint8Array(c.length / 2);
  for (let y = 0; y < c.length; y += 2) {
    g[y / 2] = parseInt(c.substr(y, 2), 16);
  }
  return g;
}
async function dZ(c) {
  ...
  const g = n9(c.slice(3));
  if (g.length < 28) {
    throw new Error("tik payload too short");
  }
  const y = g.slice(0, 12);
  const W = g.slice(12);
  const I = await crypto.subtle.importKey("raw", n9(fZ), {
    name: "AES-GCM"
  }, false, ["decrypt"]);
  const T = {
    name: "AES-GCM",
  ...
  const P = await crypto.subtle.decrypt(T, I, W);
...
async function r9(c) {
  ...
  const O = c.media.type === "show" ? String(c.media.episode.number) : "";
  const T = c.media.type === "show";
  const C = await Ui({
    ref: String(_),
    shelf: I,
    slot: O
  ...
  const V = await c.fetcher(lZ + "/tik", U);
  const Q = V == null ? undefined : V.sources;
  if (!Q || typeof Q !== "object") {
    throw new Zt("Tikki returned no sources");
  }
  const ae = hZ.filter(Le => {
    var le;
    return typeof ((le = Q[Le.source]) == null ? undefined : le.url) === "string";
  });
  const ue = await Promise.all(ae.map(async Le => {
    try {
      return await dZ(Q[Le.source].url);
    } catch {
      if ("kCwyv" === "kCwyv") {
        return "";
      }
      {
        ...
        const xe = {
          id: "primary",
          type: "file",
          flags: [ge.CORS_ALLOWED],
          captions: [],
          qualities: {}
        };
        xe.qualities.unknown = ge;
        const He = {
          stream: [xe]
        };
        return He;
  ...
  if ($.length === 0) {
    throw new Zt("Tikki found no playable sources");
  }
  const Ae = {
    embeds: $
  };
