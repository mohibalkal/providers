...
// Provider: Valley 🏔️  |  id: valley  |  type: source
// Original location: vendor.js line 22466
...
const BZ = "https://stream.fontaine.lol";
const KZ = "2f8c6a3e9d1b4c7f5a2e8d6b3f9c1a4e7d5b2f8c6a3e9d1b4c7f5a2e8d6b3f9c";
function f9(c) {
...
async function GZ(c) {
  ...
  const _ = f9(c.slice(3));
  if (_.length < 28) {
    throw new Error("valley payload too short");
  }
  const O = _.slice(0, 12);
  const T = _.slice(12);
  const P = await crypto.subtle.importKey("raw", f9(KZ), {
    name: "AES-GCM"
  }, false, ["decrypt"]);
  const U = {
    name: "AES-GCM",
  ...
  const M = await crypto.subtle.decrypt(U, P, T);
...
async function Mb(c) {
  ...
  const ae = await c.fetcher(BZ + "/valley", V);
  const ue = ae == null ? undefined : ae.sources;
  if (!ue || typeof ue !== "object") {
    throw new Zt("Valley returned no sources");
  }
  const ne = qZ.filter(Le => {
    var le;
    return typeof ((le = ue[Le.source]) == null ? undefined : le.url) === "string";
  });
  const me = await Promise.all(ne.map(async Le => {
    if ("PXEXb" !== "VrWRI") {
      try {
        return await GZ(ue[Le.source].url);
      } catch {
        return "";
      }
    } else {
      ...
      const at = {
        id: "primary",
        type: "file",
        flags: [Ze.CORS_ALLOWED],
        captions: [],
        qualities: {}
      };
      at.qualities.unknown = Ze;
      const pt = {
        stream: [at]
      };
      return pt;
  ...
  const Ae = ne.map((Le, le) => ({
    embedId: Le.id,
    url: me[le]
  })).filter(Le => Le.url);
  if (Ae.length === 0) {
    throw new Zt("Valley found no playable sources");
  }
  const Re = {
    embeds: Ae
  };
