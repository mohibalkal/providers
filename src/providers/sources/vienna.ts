...
// Provider: Vienna 🎻  |  id: vienna  |  type: source
// Original location: vendor.js line 26459
...
const Yee = "https://stream.fontaine.lol";
const $ee = "a7b07d9e8711e0626877d68026ab60ca01ce822716cdafe4e8225fdaa6a7cc76";
function V9(c) {
  const o = new Uint8Array(c.length / 2);
  for (let g = 0; g < c.length; g += 2) {
    o[g / 2] = parseInt(c.substr(g, 2), 16);
  }
  return o;
}
async function Jee(c) {
  if (!c.startsWith("vn_")) {
    return c;
  }
  const y = V9(c.slice(3));
  if (y.length < 28) {
    throw new Error("vienna payload too short");
  }
  const _ = y.slice(0, 12);
  const I = y.slice(12);
  const b = await crypto.subtle.importKey("raw", V9($ee), {
    name: "AES-GCM"
  }, false, ["decrypt"]);
  const k = {
    name: "AES-GCM",
  ...
  const M = await crypto.subtle.decrypt(k, b, I);
...
async function j9(c) {
  ...
  const W = c.media.title;
  const _ = c.media.releaseYear;
  ...
  if (!W) {
    throw new Zt("Vienna requires a title to search movibox.net");
  }
  c.progress(20);
  const b = await Ui({
    ref: String(g),
  ...
  const V = await c.fetcher(Yee + "/vienna", U);
  c.progress(80);
  const Q = V == null ? undefined : V.qualities;
  if (!Q || typeof Q !== "object" || Object.keys(Q).length === 0) {
    throw new Zt("Vienna returned no qualities");
  }
  const ae = Object.entries(Q);
  ...
  if (Object.keys($).length === 0) {
    throw new Zt("Vienna: no decodable qualities");
  }
  c.progress(95);
  const ne = {
    id: "primary",
