...
// Provider: Chase 🧣  |  id: chase  |  type: source
// Original location: vendor.js line 22213
...
async function l9(c) {
  ...
  const y = c.media.type === "show" ? String(c.media.season.number) : "";
  const W = c.media.type === "show" ? String(c.media.episode.number) : "";
  c.progress(20);
  const I = await Ui({
    ref: String(m),
    shelf: y,
  ...
  const U = await c.fetcher(MZ + "/chase", P);
  c.progress(80);
  const M = (U == null ? undefined : U.sources) ?? {};
  const V = xZ.filter(ue => {
    var $;
    return typeof (($ = M[ue.source]) == null ? undefined : $.url) === "string";
  }).map(ue => ({
    embedId: ue.id,
    url: M[ue.source].url
  }));
  if (V.length === 0) {
    throw new Zt("Chase found no playable sources");
  }
  c.progress(95);
  const ae = {
    embeds: V
