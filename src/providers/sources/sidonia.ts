...
// Provider: Sidonia  |  id: sidonia  |  type: source
// Original location: vendor.js line 24912
...
const nee = "https://stream.fontaine.lol";
const X1 = {
  source: "Hades",
  id: "sidonia-hades",
  name: "Hades",
...
async function D9(c) {
  ...
  const _ = c.media.type === "show" ? String(c.media.season.number) : "";
  ...
  const U = await c.fetcher(nee + "/sidonia", k);
  c.progress(80);
  const V = U == null ? undefined : U.sources;
  if (!V || Object.keys(V).length === 0) {
    throw new Zt("Sidonia: no sources");
  }
  const Q = [];
  for (const ne of ree) {
    const me = V[ne.source];
    if (!me) {
      continue;
    }
    const Ae = (t = me.qualities) != null && t.length ? me.qualities : me.url ? [{
      url: me.url,
      quality: me.quality,
    ...
    if (Ae.length !== 0) {
      Q.push({
        embedId: ne.id,
        url: JSON.stringify(Ae)
      });
    }
  }
  if (Q.length === 0) {
    throw new Zt("Sidonia: no playable sources");
  }
  c.progress(100);
  const ae = {
    embeds: Q
...
export const sidoniaScraper = iee;
